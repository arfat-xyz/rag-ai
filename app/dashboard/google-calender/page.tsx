...
const DashboardTwoPage = async () => {
  ...
  const files = await prisma.googleCalenderFile.findMany({
    where: {
      userEmail: user?.email,
    },
  });
  const chatbots = await prisma.googleCalenderChatbot.findMany({
    where: {
      userEmail: user?.email,
    },
  });
      ...
      <div className=" dark:text-black">
        <div className="w-full min-h-24 bg-gray-100  p-4 mt-8 rounded-lg">
          <div className="flex items-center justify-between">
            ...
            <AddPdfComponentDynamic value="google-calender" />
          </div>
          {files.length === 0 ? (
            <>
